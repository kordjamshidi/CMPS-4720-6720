...
newfile = 'autogenerated_annotations_.csv'
f = open(newfile, 'wb')
fw = csv.writer(f)
...
labels = []
csv_read_files =['manual_crop_annotations_pictures1.csv','manual_crop_annotations_pictures2.csv']
...
for x in range(1,25):    #for each whole transfer image
    ...
    if x<9: #for the first 8 images, read set 1 (tiff)s
        BeforeName = 'pictures1/' + str(x) +'A.tiff'
        AfterName = 'pictures1/' + str(x) +'B.tiff'
    else: #for the next images, read set 2, (bmp)s
        z = x-8
        BeforeName = 'pictures2/' + str(z) +'A.bmp'
        AfterName = 'pictures2/' + str(z) +'B.bmp'
